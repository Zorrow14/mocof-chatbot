...
export async function logDepositToSheet(details) {
    ...
    try {
        ...
        const row = [
            ...
            details.cabinets || '',
            // Column L, appended at the END per the rule above: the deposit
            // option the customer chose ("10% of total", "Fixed RM 1,500.00").
            details.depositOptionLabel || '',
            // Column M, likewise APPENDED at the end and not inserted — inserting
            // is what shifted Stripe Session ID from H to J last time. The product
            // this deposit is for.
            //
            // Wall bed deposits deliberately MIRROR column C here rather than
            // leaving M blank, so this column means "the product" on every row
            // and can be read on its own. A product reservation has no column C
            // to mirror and carries its own loosely-extracted label.
