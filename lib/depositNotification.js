...
export function buildDepositEmail(details = {}) {
    ...
    // The product this deposit is for, recorded for every type. Wall bed
    // deposits mirror their model into it at charge time, so a reservation email
    // names what was reserved in exactly the place a wall bed email names the
    // model — including in the subject line.
    const productLabel = details.productLabel || null;
    const model = details.wallBedModel || productLabel;
    // A reservation has no order total; saying "RM ?" would imply one exists and
    // that we failed to record it.
    const isReservation = details.depositType === 'product_reservation';
    ...
    // Which deposit option the customer chose. A fixed-amount deposit has no
    // percentage, so the "A 10% deposit" sentence and the "Deposit %" row would
    // print "?%" for it — both switch to fixed-amount wording instead. Sessions
    // from before options existed carry no option at all and render as they
    // always did, plus an honest "(not recorded)".
    const isFixedDeposit = details.depositOptionKind === 'fixed';
    const optionLabel = orNone(details.depositOptionLabel, '(not recorded)');
    const depositSentence = isFixedDeposit
        ? 'A fixed-amount deposit has been paid.'
    ...
    const lines = [
        ...
        `Product: ${orNone(productLabel, '(not recorded)')}`,
        isReservation
            ? 'Grand total: (not applicable — fixed reservation deposit)'
            : `Grand total: RM ${orNone(details.grandTotal, '?')}`,
        `Deposit option: ${optionLabel}`,
    ...
    const rows = [
        ...
        sectionRow('Deposit'),
        ...(isFixedDeposit ? [] : [fieldRow('Deposit %', `${orNone(details.depositPercent, '?')}%`)]),
        fieldRow('Quote ref', orNone(details.quoteRef, '(none)')),
        fieldRow('Wall bed model', orNone(details.wallBedModel, '(none)')),
        fieldRow('Product', orNone(productLabel, '(not recorded)')),
        fieldRow('Grand total', isReservation
            ? '(not applicable — fixed reservation deposit)'
            : `RM ${orNone(details.grandTotal, '?')}`),
        fieldRow('Deposit option', optionLabel),
